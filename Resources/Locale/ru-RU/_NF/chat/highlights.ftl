highlights-station-representative = "ПФ", Представитель Фронтира, Представитель
highlights-station-traffic-controller = "Дисп", Диспетчер
highlights-valet = Камердиннер
highlights-mail-carrier = Почтальон
# janitor using upstream definition
highlights-security-guard = "СБ", Офицер АФ, Офицер, Охранник АФ, Охранник
highlights-director-of-care = "ММО", Менеджер, Менеджер Медицинских Операций

highlights-cadet = Кадет, "ТСФ"
highlights-deputy = Рейнджер, Рейндж, "ТСФ"
highlights-detective = Детектив, "Дек", "ТСФ"
highlights-brigmedic = Коронер, Коронёр, "ТСФ"
highlights-sergeant = Лейтенант, Лейт, "ТСФ"
highlights-bailiff = Бейлиф, Бейл, "ТСФ"
highlights-public-affairs-liaison = Представитель, Представитель ЮрДеп, "ПЮД", "ТСФ"
highlights-sheriff = Командующий офицер, КО, "ТСФ"

highlights-prisoner = Заключенный, Зэк
highlights-pirate = Пират
highlights-pirate-first-mate = Пират, "Боц", Боцман
highlights-pirate-captain = Пират, "Кэп", Капитан
