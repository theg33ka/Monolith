...
public sealed partial class MindSystem : SharedMindSystem
{
    ...
    public override bool TryGetMind(NetUserId user, [NotNullWhen(true)] out EntityUid? mindId, [NotNullWhen(true)] out MindComponent? mind)
    {
        ...
        {
            DebugTools.Assert(!_players.TryGetPlayerData(user, out var data) || data.ContentData()?.Mind == mindId);
            return true;
        }

        DebugTools.Assert(!_players.TryGetPlayerData(user, out var data2) || data2.ContentData()?.Mind == null);
        return false;
    }
