...
public sealed partial class MarkingPicker : Control
{
    ...
    public void Populate(string filter)
    {
        ...
        foreach (var marking in sortedMarkings)
        {
            ...
            var item = CMarkingsUnused.AddItem($"{GetMarkingName(marking)}", GetMarkingPreviewTexture(marking));
    ...
    public void PopulateUsed()
    {
        ...
        foreach (var marking in _currentMarkings.GetReverseEnumerator(_selectedMarkingCategory))
        {
            ...
            {
                Text = text,
                Icon = GetMarkingPreviewTexture(newMarking),
                Selectable = true,
                Metadata = newMarking,
    ...
    private Texture GetMarkingPreviewTexture(MarkingPrototype marking)
    {
        var sprite = marking.Sprites[0];
        var spriteSystem = IoCManager.Resolve<IEntitySystemManager>().GetEntitySystem<SpriteSystem>();
        var state = spriteSystem.RsiStateLike(sprite);

        // Some animated markings (notably tails) have empty early frames and/or
        // empty south-facing preview frames. Use a later frame and north direction
        // to produce a stable non-empty icon in the picker.
        if (state.IsAnimated)
        {
            var previewFrame = Math.Min(2, state.AnimationFrameCount - 1);
            return state.GetFrame(RsiDirection.North, previewFrame);
        }
